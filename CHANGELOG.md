...
- [优化] 平台模型列表按当前用户分组过滤，模型未向所在分组开放时显示明确的权限说明及后台配置的管理员联系方式。

## v0.19.11 - 2026-09-21

- [优化] 移除与右侧配置按钮功能重复的顶部“配置”导航入口。

## v0.19.10 - 2026-09-21

- [修复] 登录与注册入口改为一体化拼接按钮组，并提高深色模式下注册按钮的文字对比度。

## v0.19.9 - 2026-09-21

- [优化] 前台未登录状态改为显示清晰的“登录”和“注册”文字按钮，替代含义不明显的单一登录图标。

## v0.19.8 - 2026-09-21

- [调整] 管理员登录与版本更新入口迁移至独立的 `/admin` 后台，前台不再展示后台入口和版本信息。

## v0.19.7 - 2026-09-21

- [修复] 顶部文档入口改为灵舟自有平台介绍页，不再依赖 `docs.canvas.best`。

## v0.19.6 - 2026-09-21

- [优化] 无限画布文档入口首页改为灵舟 AI 平台介绍与导流页，注册、模型价格和使用文档统一跳转算力平台。

## v0.19.5 - 2026-09-20

- [新增] 代理邀请客户同步接口增加当前余额、充值、区间消费、失败返还、净消耗及最近活动汇总，并保持按邀请 Token 隔离。

## v0.19.4 - 2026-09-20

- [修复] 邀请注册链接进入后在当前浏览器会话保留邀请 Token，避免页面跳转或已有登录状态重定向导致注册时丢失邀请归属和赠送算力点。

## v0.19.3 - 2026-09-20

- [修复] 后台使用日志的时间范围统一按 UTC 查询，避免北京时间“今天”筛选遗漏当日流水。

## v0.19.2 - 2026-09-20

- [新增] 代理专属邀请注册支持按运维端配置赠送算力点，默认 50 点、最高 100 点，并以注册事务和独立流水保证只赠送一次。
...
- [修复] 视频异步任务不再因前端固定轮询次数耗尽而误报失败；长任务会持续后台查询并自动回填，平台任务固定原渠道查询，明确失败时幂等退款。
...
- [修复] 将 `tejiasd-mini-720p` 等未包含 `video` 字样的已知视频模型识别为视频模型，避免后台配置后前端视频创作台无法选择。

## v0.18.5 - 2026-09-15

- [修复] 后台已配置平台模型且浏览器本地渠道未配置 API Key 时，前端默认使用平台模型列表，避免视频创作台只显示本地默认示例模型。

## v0.18.4 - 2026-09-15

- [修复] 修复 `seedance-2.0-mini` 等 Seedance 模型未包含 `video` 字样时，被前端误判为文本模型，导致视频创作台无法选择的问题。

## v0.18.3 - 2026-09-15

- [优化] 前端入口页返回 `no-cache`，避免发布新路由后浏览器继续使用旧入口资源。

## v0.18.2 - 2026-09-15

- [修复] 修复代理专属邀请链接打开 `/register` 时被前端路由渲染为 404，并自动打开注册窗口。

## v0.18.1 - 2026-09-15

- [新增] 注册支持代理邀请 ref，并向运维系统回调以自动建立无限画布客户归属。
- [新增] 算力中心增加云猫在线充值入口，支持购买 10/50/100 元兑换码后完成充值。

+ [修复] 适配 Pidoi `grok-imagine-video-1.5-preview` 视频接口，使用 JSON 请求及标准字段，避免 multipart 请求解析失败。
+ [新增] 支持用户分组算力点倍率与渠道分组访问控制，管理员可在用户和渠道设置中配置。
+ [修复] 生图与生视频工作台的模型选择仅展示对应能力的模型，并修正错误的默认模型归类。
