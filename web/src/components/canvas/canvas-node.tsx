...
function LoadingContent({ node, theme }: Pick<NodeContentRendererProps, "node" | "theme">) {
    const { t } = useTranslation();
    const label = node.metadata?.videoTaskState === "background" ? t("canvas.node.videoBackground") : node.metadata?.videoTaskState === "query_interrupted" ? t("canvas.node.videoQueryInterrupted") : t("canvas.node.generating");
    return (
        <div className="flex h-full w-full flex-col items-center justify-center gap-3" style={{ color: theme.node.activeStroke }}>
            <div className="size-10 animate-spin rounded-full border-2" style={{ borderColor: theme.node.stroke, borderTopColor: theme.node.activeStroke }} />
            <span className="px-3 text-center text-[10px]">{label}</span>
        </div>
    );
}
    ...
    return (
        <div className="flex max-w-[260px] flex-col items-center gap-3 px-5 text-center">
            ...
            >
                ...
                {node.metadata?.videoTaskId ? t("canvas.nodeToolbar.queryVideoTask") : t("canvas.node.retry")}
            </button>
        </div>
    );
}
