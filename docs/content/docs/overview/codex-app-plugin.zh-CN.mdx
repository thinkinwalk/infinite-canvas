...
插件会启动本地 Canvas Agent，读取连接地址和 token，然后在右侧打开 `https://studio.lingzhouai.com` 并自动新建、连接画布。只有明确要求公共演示站时才打开 `https://canvas.best`；如果已经打开生产站点的画布，插件优先操作当前页面，不会重复跳转。只有明确要求使用本地项目时，插件才会启动本地前端。
