...
```
https://raw.githubusercontent.com/thinkinwalk/infinite-canvas/plugins-dist/official-plugins.json
```

清单里每条的 `entry`(相对文件名)由前端解析成与清单同目录的绝对 URL,再走既有 URL 安装流程。GitHub Raw 使用短时缓存，发布后可通过清单响应确认当前版本。
